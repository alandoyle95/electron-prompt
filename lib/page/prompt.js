...
const promptSubmit = () => {
    ...
    if(promptOptions.type === 'input') {
        ...
        if(promptOptions.selectMultiple) {
            data = [];
            $('select').find('option:selected').each((k,o) => {
                if($(o).html() != '')
                    data.push($(o).attr('value'));
            });
        } else {
            data = dataEl.value;
        }
...
docReady(() => {
    ...
    if(promptOptions.type === 'input') {
        ...
        if(promptOptions.selectMultiple)
            dataEl.setAttribute('multiple', 'multiple');
        let optionEl;

        //append a blank option to the list
        ...
        for(let k in promptOptions.selectOptions) {
            ...
            if(promptOptions.value) {
                ipcRenderer.send('prompt-log:'+promptId, promptOptions.value+" includes "+k+" = "+promptOptions.value.includes(k));
                if (promptOptions.value.includes(k)) {
    ...
    dataEl.focus();
    ipcRenderer.send('prompt-log:'+promptId, dataEl);
